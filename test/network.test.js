...
test('a fresh node reports itself online at height 0', () => {
  const { store, config } = testNode({ CHATSCAN_SEALER_ENABLED: 'true' });
...
test('an idle chain stays online even once its tip is old', () => {
  ...
  const snapshot = networkSnapshot({ store, config, now: block.timestamp + 60_000 });
  assert.equal(snapshot.status, 'online');
  assert.equal(snapshot.unconfirmed.count, 0);
  assert.ok(snapshot.tipAgeMs >= 60_000);
});

test('a backed-up mempool with a stale tip reports degraded', () => {
  const { store, node, config } = testNode({
    CHATSCAN_SEALER_ENABLED: 'true',
    CHATSCAN_BLOCK_INTERVAL_MS: '1000',
  });
  node.submit(sampleSubmission());
  const block = node.sealBlock({ now: 1_000_000 });
  node.submit(sampleSubmission({ nonce: 'aa' }));

  assert.equal(networkSnapshot({ store, config, now: block.timestamp + 500 }).status, 'online');
  assert.equal(networkSnapshot({ store, config, now: block.timestamp + 60_000 }).status, 'degraded');
});

test('a disabled sealer reports paused', () => {
  const { store, config } = testNode({ CHATSCAN_SEALER_ENABLED: 'false' });
  assert.equal(networkSnapshot({ store, config }).status, 'paused');
});

test('the fee estimate rises with mempool pressure', () => {
...
test('the snapshot counts 24-hour activity and per-record averages', () => {
  const { store, node, config } = testNode();
